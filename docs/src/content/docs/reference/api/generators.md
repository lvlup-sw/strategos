...
The saga document stores `Phase` as a **name** under System.Text.Json and as an **ordinal** under Newtonsoft. A member reorder (for example the document-order restore in 3.0.0-rc.1) is safe only for the name representation. See [Phase Enum Persistence](/reference/phase-persistence).
