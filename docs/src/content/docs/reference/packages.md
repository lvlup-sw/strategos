...
See the [Ontology Reference](/reference/ontology) for the modeling guide.
...
The analyzers report the `AONT` diagnostic series; see [AONT001-AONT099](/reference/diagnostics/aont-001-aont-099), [AONT100-AONT199](/reference/diagnostics/aont-100-aont-199), and the [AONT200 series](/reference/diagnostics/aont-200-series).
...
:::note[Development dependency]
Analyzers run during build and produce no runtime overhead.
:::
...
See the [Npgsql provider reference](/reference/ontology/npgsql) for schema and index setup.
...
:::caution[Deprecated]
`Strategos.Rag` is deprecated. Use `Strategos.Ontology` with an `IObjectSetProvider` instead — it covers the same retrieval need with a typed object model, and it is where the pgvector and embedding work landed.
:::
