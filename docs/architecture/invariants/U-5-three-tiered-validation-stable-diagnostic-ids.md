...
2. **Roslyn analyzer** — compile-time diagnostics via `WorkflowDiagnostics.cs` (AGWF catalog through AGWF045 as of 3.0, identities in `AgwfCodes.g.cs`) and `OntologyDefinitionAnalyzer.cs` (AONT001..035).
...
- `src/Strategos.Contracts/Generated/AgwfCodes.g.cs` — AGWF identities through AGWF045 as of 3.0 (TypeSpec-canonical; descriptors in `WorkflowDiagnostics.cs` consume these constants)
...
- `src/Strategos.Ontology.Generators/Diagnostics/OntologyDiagnosticIds.cs:5-49` — AONT001..037 plus AONT200..221 (action calculus) covering core, preconditions, lifecycle, derivation, interface actions, extension points
