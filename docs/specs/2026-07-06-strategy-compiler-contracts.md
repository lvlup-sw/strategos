...
**Acceptance criteria:**
- **Partition assertion:** every `WorkflowCorpus` fixture (≥100) lands in exactly one bucket — (a) importable subset ⇒ export→import→compile succeeds, or (b) carrier-bearing (branch/repeatUntil/approval-with-context/…) ⇒ the specific DR-14 diagnostic fires. A fixture in neither bucket fails the gate. (The corpus's 26 branch, 26 repeatUntil, and context-bearing awaitApproval fixtures populate bucket (b) — the rejection proof is not vacuous.) The `awaitApproval` family straddles BOTH buckets and is classified per case: context-bearing cases are carriers (bucket b, AGWF031); the CONTEXT-FREE cases (`approval-free-*`) are importable (bucket a). **Context-free approval (bucket a) — CLOSED (M10, FIX-5):** a builder-produced context-free approval mints a GUID `approvalPointId` (`ApprovalDefinition.Create`), and the import bridge (`WireToModelBridge.MapApprovals`) used to feed that raw GUID to `ApprovalModel.Create` as the approval-point NAME; a digit-leading GUID is not a valid C# identifier, so the generator threw `CS8785` and no saga was emitted (the crash was intermittent — only digit-leading GUIDs fail `SyntaxFacts.IsValidIdentifier`). FIX-5 DERIVES a valid identifier from the approver type via the shared `ApprovalPointNaming.Derive` — the SAME derivation the C#-authoring `ApprovalExtractor` uses — and keeps the GUID for wire identity only, so the context-free approval now lowers a saga (bucket a). That the lowered approval saga COMPILES (its resume command references the now-present `Strategos.Models.ApprovalDecision`) is proven on a real reference graph by the `Strategos.Generators.Behavioral.Tests` build (`roundtrip-approval.workflow.json` → `AddRoundtripApprovalImportWorkflow()`, exercised Docker-free by `RoundTripApprovalImportCompileTests`).
- **Behavioral equivalence:** for each importable combinator family, a hand-authored `[Workflow]` C# source twin and its exported JSON compile to behaviorally-identical sagas on the real host (the corpus itself is runtime builder invocations, not parseable literal source — twins are the honest baseline). **DEFERRED (fork-join family) — pending strategos#155:** a C# `.Fork(...).Join<T>().Finally<TEnd>()` twin does NOT complete on the current generator (C#-authoring appends the fork-path steps after the top-level terminal, so the terminal is not last and its completed handler chains back to a fork-path step instead of `MarkCompleted()` — a terminal-detection ordering bug, strategos#155). Shipped with a **machine-checked deferral**: the fork-join twin-equivalence proof lives in the suite as a SKIPPED test (`RoundTripBehavioralTests.ForkJoinCSharpTwin_RunsIdentically_ToJsonImport`, `[Skip("blocked on strategos#155 …")]`) that goes green automatically once its blocker lands. **Updated 2026-08-23 (v2.11.0 milestone; first published in 3.0.0-rc.1):** strategos#155 is FIXED and the C# twin now passes in isolation; the `[Skip]` remains for strategos#180 (shared-host timeout on the JSON-import half in a full-class run). Cite #180 as the blocker from here on. The fork-join **JSON-import** side is fully proven (runtime-run + IR fidelity + partition membership); only the C#-twin ≡ import equivalence is deferred.
...
**Acceptance status (fork-join twin equivalence):** DEFERRED, blocked on strategos#180 (shared-host timeout in the full-class run). strategos#155, the fork-join terminal-detection ordering defect originally cited here, was fixed under the v2.11.0 milestone and first published in 3.0.0-rc.1. Verified by a SKIPPED, machine-checked twin-equivalence test rather than a manual note; all other importable families (startWith, then, onFailure, config) and the fork-join JSON import are fully verified. Do not close DR-15's fork-join twin sub-item until strategos#180 lands and the `[Skip]` is removed and the test passes.
