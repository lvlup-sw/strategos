...
- **Behavioral equivalence:** for each importable combinator family, a hand-authored `[Workflow]` C# source twin and its exported JSON compile to behaviorally-identical sagas on the real host (the corpus itself is runtime builder invocations, not parseable literal source — twins are the honest baseline). **DEFERRED (fork-join family) — pending strategos#155:** a C# `.Fork(...).Join<T>().Finally<TEnd>()` twin does NOT complete on the current generator (C#-authoring appends the fork-path steps after the top-level terminal, so the terminal is not last and its completed handler chains back to a fork-path step instead of `MarkCompleted()` — a terminal-detection ordering bug, strategos#155). Shipped with a **machine-checked deferral**: the fork-join twin-equivalence proof lives in the suite as a SKIPPED test (`RoundTripBehavioralTests.ForkJoinCSharpTwin_RunsIdentically_ToJsonImport`, `[Skip("blocked on strategos#155 …")]`) that goes green automatically once strategos#155 lands. The fork-join **JSON-import** side is fully proven (runtime-run + IR fidelity + partition membership); only the C#-twin ≡ import equivalence is deferred.
...
**Acceptance status (fork-join twin equivalence):** DEFERRED, blocked on strategos#155 (fork-join terminal-detection ordering). Verified by a SKIPPED, machine-checked twin-equivalence test rather than a manual note; all other importable families (startWith, then, onFailure, config) and the fork-join JSON import are fully verified. Do not close DR-15's fork-join twin sub-item until strategos#155 lands and the `[Skip]` is removed and the test passes.
