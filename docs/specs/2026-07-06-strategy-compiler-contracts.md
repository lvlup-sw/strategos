...
**Acceptance criteria:**
- **Partition assertion:** every `WorkflowCorpus` fixture (≥100) lands in exactly one bucket — (a) importable subset ⇒ export→import→compile succeeds, or (b) carrier-bearing (branch/repeatUntil/approval-with-context/…) ⇒ the specific DR-14 diagnostic fires. A fixture in neither bucket fails the gate. (The corpus's 26 branch, 26 repeatUntil, and context-bearing awaitApproval fixtures populate bucket (b) — the rejection proof is not vacuous.) **Known gap (context-free approval, bucket a):** a context-free approval is importable by design (DR-12/DR-14), but a BUILDER-produced one cannot currently be exercised in the corpus — the builder mints a GUID `approvalPointId` (`ApprovalDefinition.Create`) and the import bridge (`WireToModelBridge.MapApprovals`) feeds that raw GUID to `ApprovalModel.Create`, which requires a valid C# identifier, so the generator throws `CS8785` (the C#-authoring path avoids this by deriving a valid name via `ApprovalExtractor.GenerateApprovalPointName` and ignoring the GUID). Closing this needs the import bridge to derive/sanitize the approval name the same way (import layer) — tracked as a follow-on, not fixed in this test-hardening pass.
