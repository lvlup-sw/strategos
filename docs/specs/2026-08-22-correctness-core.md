...
- INV-3's own catalog is amended via `invariants_amend` (12 sites across three files), including the executable grep gate at `deterministic-checks.md:101`, whose deny-list must gain `2025-11-25` and whose path scope excludes the `Agents.Mcp` README it should cover.
