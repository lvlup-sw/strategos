...
- `SagaDocument` (non-EventSourced) output for a linear workflow with no off-main-flow steps is unchanged against the task-030 baseline (regression guard).
  > **Corrected at implementation:** *byte*-identical across commits is not achievable and the requirement should never have said so. The emitted `[GeneratedCode("LevelUp.Strategos", "<version>")]` resolves from the generator assembly's `AssemblyInformationalVersionAttribute` (`Helpers/GeneratorMetadata.cs:31-39`), which MinVer derives from commit height — so it moves even when no generator source changes. That single token is held as a placeholder and substituted identically on both sides (with LF normalization); everything else is compared exactly, and a third test pins that the version argument is the **only** thing normalized away.
...
**Second amendment, wave 3 — #175 is two defects, and the classification closed the headline one.** Measured on the integrated branch *before* any change to the branch emitter: the three real-host branch tests pass **5/5**. So the unbounded cascade — terminal chains into a branch-path step, which rejoins at the terminal, saga laps forever — was closed by the **off-main-flow classification** (DR-1), not by anything in this requirement. Once branch-case steps stop being main-flow successors, the declared terminal resolves `nextStepName = null` and marks completed.

What this requirement actually closes is a **second, latent** defect that the terminal-case exclusion was hiding: the path-end handler deciding from the branch-level rejoin flag. Both changes are needed — widening the lookup without the case-level decision *introduces* a regression — but the rejoining twin is coverage for the cascade, not proof of this fix. The only oracle that can fail on the case-level decision is the terminal-case guard under the widening-only mutation, and it does. Recorded so a later reader does not mistake one proof for the other.

This is the clearest evidence for the slice's approach: the class fix closed an instance that was filed separately and was never aimed at.

**Premise corrected at implementation (wave 2):** a terminal `.Complete()` case is **not** routed to the declared terminal today — on the unfixed generator its last step already calls `MarkCompleted()`, identically for a `When(...)` and an `Otherwise(...)` terminal case. That works by accident: `BuildBranchPathInfo` excludes terminal cases, so the step never reaches the path-end handler and falls through to the ordinary step handler, where `isLastStep` is positional and branch-path steps sit after the terminal in `StepNames`. The requirement's *conclusion* survives and gets sharper — widening the table alone starts routing that step through the path-end handler, where the branch-level rejoin flag is true, **newly breaking a case that currently works**. So the widening and the case-level decision must land together, and the currently-passing terminal-case behaviour is a live regression guard, not a test to quarantine.
...
- **Container-runtime absence is distinguishable from regression.** A Docker-less developer sees the whole behavioral suite red — indistinguishable from a real break during this slice.
  > **Corrected at implementation: the throw site is the constructor, not `StartAsync`.** `PostgreSqlBuilder(...).Build()` runs in `PostgresFixture..ctor()` and Testcontainers validates the endpoint there, surfacing as `Failed to expand data source` for every test at once. `InitializeAsync()` is only `container.StartAsync()`. Wrapping `StartAsync` alone would leave the diagnostic unreachable in exactly the scenario it exists for — both sites need it.
  > A second trap: a **set** `DOCKER_HOST` is not a **reachable** one. Deciding "resolved" from the variable being present produces an actively misleading message for a bogus socket path; reachability must be probed, and a non-unix endpoint reported as unprobeable rather than guessed at. The suite emits an unambiguous diagnostic when the daemon is unreachable, or the task explicitly records Docker as a dispatch prerequisite verified before implementation starts.
...
**A runaway saga outlives the test that started it** (wave 2, measured). A non-terminating workflow keeps incrementing the shared `WorkflowInvocationLog` after its test has failed — one branch test observed **90,124** invocations. So a whole-log `TotalCount` assertion is only meaningful while *every* workflow on the host terminates, which is exactly what this slice is fixing. Assert exact per-step counts, never a log total. This is very likely the same failure class as the wave-1 second-run observation below.

**A separate compile-time defect was found and filed as #179.** A `Branch` on a `bool` discriminator with both cases declared emits an unreachable `_ =>` arm and the generated saga **does not compile** (CS8510). It is out of scope here — a different failure mode in the same emitter family — but it constrains this slice's fixtures: a branch fixture must use an enum discriminator until #179 lands. It survived undetected because the one fixture carrying that shape is parse-and-emit-string tested, never compiled.

**A warning for the behavioral tasks (012, 016, 017, 023), recorded from wave 1.** `RoundtripForkImportSaga` was observed not completing on a *second* run within the same host process (30s timeout) while passing in isolation. That is **one reproduction, not a characterised bug** — but tasks 012, 016 and 017 all extend that same shared host, so a twin that fails only in a full class run should be suspected here before the generator is. Seed the shared invocation log directly rather than building on a prior run's state.
