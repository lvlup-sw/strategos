...
public sealed class StepConfigParityTests
{
    ...
    public async Task StepConfigParity_EveryMember_IsLoweredOrDeferred()
    {
        ...
        foreach (var (member, proof) in Lowered)
        {
            ...
            var proofPath = Path.Combine(repoRoot, "tests", proof.BehavioralTestFile);
        ...
        foreach (var (member, proof) in OccurrenceMetadata)
        {
                     ...
                     })
            {
                var inspection = BehavioralProofInspector.InspectFile(
                    Path.Combine(repoRoot, "tests", file), test.Split('.').Last());
    ...
    /// <summary>
    /// Walks up from the running test assembly's directory to the repository root — the
    /// directory that contains <c>strategos.slnx</c> — so the relative
    /// <see cref="LoweredProof.BehavioralTestFile"/> paths can be resolved at test runtime
    /// regardless of the build output layout. Those paths are relative to <c>tests/</c>.
    /// </summary>
    ...
    private static string FindRepoRoot()
    {
        ...
        while (dir is not null)
        {
            ...
            {
                // The solution file sits at the repository root. Every path below names its
                // tree ("src" or "tests") explicitly, so moving a project fails on a missing
                // directory instead of resolving somewhere plausible and scanning nothing.
                return dir.FullName;
            }
