...
/// </para>
/// <para>
/// This gate reads only the <c>[2.13.0]</c> section, so an entry in an older section does not
...
{
    /// <summary>The identifiers that must appear in the 2.13.0 section.</summary>
    ...
    private static async Task<string> ReadReleaseSectionAsync()
    {
        ...
        // Carve between "## [2.13.0]" and the next "## [" heading.
        ...
        if (!match.Success)
        {
            throw new InvalidOperationException(
                "CHANGELOG.md has no '## [2.13.0]' section followed by a later release heading.");
        }
