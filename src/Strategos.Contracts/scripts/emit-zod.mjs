...
/**
 * Lowers one schema node into a TypeScript TYPE (#223).
 *
 * This is a second projection of the same document, not a pretty-printer for the
 * Zod one. It exists because TypeScript cannot infer a const whose initializer
 * reads a sibling that reads it back: every schema in a reference cycle would
 * otherwise carry `z.ZodType<unknown>`, and the cost lands on the headline type
 * — `StepDefinition` is a union at the cycle's entry point, so
 * `WorkflowDefinitionV1['steps']` inferred as `unknown[]`. Runtime validation was
 * completely correct and the static type said nothing.
 *
 * It carries the SAME totality obligation as `compile`. A shape it cannot render
 * stops the emit. It never renders `any`, which is the exact analogue of a
 * dropped constraint in a schema emitter: invisible, and only ever too
 * permissive.
 */
function typeExpression(node, context) {
  // The empty schema admits any value. `unknown` is its honest type — not `any`,
  // which would let a consumer read members the contract never promised.
  if (!Object.keys(node).some((key) => LOWERED.has(key))) {
    return "unknown";
  }
  ...
  if ("$ref" in node) {
    ...
    // A sibling `minProperties` refines the VALUE, never the type, so the
    // reference alone is the whole type here.
  ...
  if ("enum" in node) {
    return node.enum.map((member) => JSON.stringify(member)).join(" | ");
  }

  switch (node.type) {
    case "object":
      return objectType(node, context);
    case "array":
      return `Array<${typeExpression(node.items, { ...context, where: `${context.where}[]` })}>`;
    case "string":
      return "string";
    case "integer":
    case "number":
    ...
    case "null":
      return "null";
    default:
      fail(`${context.where}: unsupported type '${JSON.stringify(node.type)}' in the type projection`);
      return "";
  }
}

/** Renders an object node as a TS members block, or as a record when it is open. */
function objectType(node, context, indent = "  ") {
  if ("unevaluatedProperties" in node) {
    const value = typeExpression(node.unevaluatedProperties, {
      ...context,
  ...
  const members = entries.map(([name, schema]) => {
    const type = typeExpression(schema, { ...context, where: `${context.where}.${name}` });
    const mark = required.has(name) ? "" : "?";
    return `${indent}${JSON.stringify(name)}${mark}: ${type};`;
  });
...
/**
 * Renders the authored type declaration for one document in a reference cycle.
 *
 * An OBJECT becomes an `interface`, because TypeScript resolves a circular
 * reference through an interface and rejects the same cycle written as a type
 * alias. Everything else — a union at a cycle's entry point, most importantly —
 * becomes a type alias, which is legal precisely because its cycle passes
 * through the interfaces its arms name.
 */
function typeDeclaration(name, document, context) {
  const where = { ...context, self: name, where: name };
  if (document.type === "object" && !("unevaluatedProperties" in document) && !("$ref" in document)) {
    return `export interface ${name} ${objectType(document, where)}`;
  }

  return `export type ${name} = ${typeExpression(document, where)};`;
}

function moduleSource(name, document, context, rules) {
  const expression = compile(document, { ...context, self: name, where: name });

  // #223: a schema inside a reference cycle gets an AUTHORED type, because its
  // own cannot be inferred. Everything else keeps `z.infer`, which preserves the
  // concrete Zod class (and so `.shape`, `.extend()` and the rest of its surface)
  // that a `z.ZodType<T>` annotation would erase. The cycle is the only place
  // that trade is forced, so it is the only place it is made.
  const cyclic = context.cycles.get(name) !== undefined;
  const declaration = cyclic
    ? typeDeclaration(name, document, context)
    : null;
  ...
  if (cyclic) {
    const typeTargets = [...context.typeImports].filter((target) => target !== name).sort();
    for (const target of typeTargets.reverse()) {
      importLines.unshift(`import type { ${target} } from "./${target}.js";`);
    }
  }
  ...
  // own type cannot be inferred from its initializer. The authored declaration
  // breaks that loop, and the annotation binds the schema to it.
  if (cyclic) {
    lines.push(declaration);
    lines.push("");
    lines.push(`export const ${name}Schema: z.ZodType<${name}> = ${body};`);
  } else {
...
async function main() {
  ...
  for (const name of names) {
    const context = { documents, cycles, imports: new Set(), typeImports: new Set() };
